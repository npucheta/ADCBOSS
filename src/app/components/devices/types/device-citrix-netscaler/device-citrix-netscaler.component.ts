...
@Component({
    selector: 'app-device-citrix-netscaler',
    templateUrl: './device-citrix-netscaler.component.html',
    styles: []
})
export class DeviceCitrixNetscalerComponent implements TypeDeviceInterface, OnInit, AfterViewInit, OnDestroy {
    ...
    data: ChannelService;
    readonly SELF_CHANNEL = channels.CHANNEL_FOR_APP_DEVICE_CITRIX_NETSCALER; // this should be unique

    constructor(private _cd: ChangeDetectorRef, public sharingDataService: SharingDataService,public netscalerService:NetscalerService) {
    }
            ...
            .subscribe(data => {
                if (isForMe(this.SELF_CHANNEL, data.to)) {
                    if (environment.DEBUG) {
                        console.log('from init CITRIX action ==>', data);
                    }
                    this.data = data;
    ...
    public save() {
        ...
        this.parent.loadbalancerService = this.netscalerService;
