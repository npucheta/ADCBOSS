...
@Component({
    selector: 'app-device-cisco-css',
    templateUrl: './device-cisco-css.component.html',
    styles: []
})
export class DeviceCiscoCssComponent implements TypeDeviceInterface, OnInit, AfterViewInit, OnDestroy {
    ...
    data: ChannelService;
    readonly SELF_CHANNEL = channels.CHANNEL_FOR_APP_DEVICE_CISCO_CSS; // this should be unique

    constructor(private _cd: ChangeDetectorRef, public sharingDataService: SharingDataService,public ciscoCSSService:CiscoCSSService) {
    }
            ...
            .subscribe(data => {
                if (isForMe(this.SELF_CHANNEL, data.to)) {
                    if (environment.DEBUG) {
                        console.log('from init CISCO action ==>', data);
                    }
                    this.data = data;
    ...
    public save() {
        ...
        this.parent.loadbalancerService = this.ciscoCSSService;
