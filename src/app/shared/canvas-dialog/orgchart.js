export default class OrgChart extends HTMLElement {
        ...
        if (opts.toggleSiblingsResp && (typeof opts.ajaxURL === 'undefined' ||
            ...
            if (prevSib) {
                if (prevSib.classList.contains('hidden')) {
                    leftEdge.classList.add('fa-angle-left');
                    leftEdge.classList.remove('fa-angle-right');
                } else {
                    leftEdge.classList.add('fa-angle-right');
                    leftEdge.classList.remove('fa-angle-left');
                }
            }
            if (nextSib) {
                if (nextSib.classList.contains('hidden')) {
                    rightEdge.classList.add('fa-angle-right');
                    rightEdge.classList.remove('fa-angle-left');
                } else {
                    rightEdge.classList.add('fa-angle-left');
                    rightEdge.classList.remove('fa-angle-right');
                }
            }
        } else {
            ...
            leftEdge.classList.toggle('fa-angle-right', sibsVisible);
            leftEdge.classList.toggle('fa-angle-left', !sibsVisible);
            rightEdge.classList.toggle('fa-angle-left', sibsVisible);
            rightEdge.classList.toggle('fa-angle-right', !sibsVisible);
        }
    }
    ...
    _hoverNode(event) {
        ...
        if (event.type === 'mouseenter') {
            if (topEdge) {
                ...
                topEdge.classList.toggle('fa-angle-up', !flag);
                topEdge.classList.toggle('fa-angle-down', flag);
            }
            if (bottomEdge) {
                flag = this._getNodeState(node, 'children').visible;
                bottomEdge.classList.toggle('fa-angle-down', !flag);
                bottomEdge.classList.toggle('fa-angle-up', flag);
            }
            if (leftEdge) {
            ...
            Array.from(node.querySelectorAll(':scope > .edge')).forEach((el) => {
                el.classList.remove('fa-angle-up', 'fa-angle-down', 'fa-angle-right', 'fa-angle-left');
            });
        }
    }
    ...
    showSiblings(node, direction) {
        ...
        this._one(siblings[0].querySelector('.slide'), 'transitionend', function () {
            ...
            if (this._isInAction(node)) {
                this._switchHorizontalArrow(node);
                node.querySelector('.topEdge').classList.remove('fa-angle-up');
                node.querySelector('.topEdge').classList.add('fa-angle-down');
            }
        }, this);
    }
    ...
    // build the sub nodes of specific node
